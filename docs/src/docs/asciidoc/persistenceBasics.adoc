A key thing to remember about GORM is that under the surface GORM is using http://www.hibernate.org/[Hibernate] for persistence. If you are coming from a background of using http://api.rubyonrails.org/classes/ActiveRecord/Base.html[ActiveRecord] or http://www.mybatis.org/[iBatis/MyBatis], Hibernate's "session" model may feel a little strange.
