Partitioned multi-tenancy is when a discriminator column is used each multi-tenant class in order to partition the data.
...
In order to activate discriminator-based multi-tenancy you need to see the multi-tenancy mode to `DISCRIMINATOR` in your configuration and supply a link:../api/org/grails/datastore/mapping/multitenancy/TenantResolver.html[TenantResolver]:
